...
# sys.stdout = codecs.getwriter('utf8')(sys.stdout.buffer)
cachefile_directory = "206project2_caching.json"

try:
    cachefile = codecs.open(cachefile_directory,'r')
    cachecontent = cachefile.read()
    CACHE_DICTION = json.loads(cachecontent)
except:
    CACHE_DICTION = {}

def twittercache(searchword):
    identifier = "twitter_" + searchword
    if identifier in CACHE_DICTION:
        print("Using cached data.\n---------------------")
        results = CACHE_DICTION[identifier]
    else:
        print('Getting cached data.\n---------------------')
        results = api.search(q=searchword)
        CACHE_DICTION[identifier] = results
        f = codecs.open(cachefile_directory,'w')
        f.write(json.dumps(CACHE_DICTION))
twittercache("University of Michigan")
...
def find_urls(string):
    urls = re.findall(r"(http[s]?://+[A-Za-z.]{2,})", string, re.MULTILINE)
    return urls
...
def get_umsi_data():
    if "umsi_directory_data" in CACHE_DICTION:
        print("Using cached UMSI directory data.\n---------------------")
    else:
        CACHE_DICTION["umsi_directory_data"] = []
        print("Getting UMSI directory data.\n---------------------")
        i = 0
        while i<12:
            if i == 0:
                ...
                response = requests.get(base_url,headers={'User-Agent': 'SI_CLASS'})
            CACHE_DICTION["umsi_directory_data"].append(response.text)
            f = codecs.open(cachefile_directory,'w')
            f.write(json.dumps(CACHE_DICTION))
            i += 1
...
umsi_titles = {}
while page < 12:
    soup = BeautifulSoup(CACHE_DICTION["umsi_directory_data"][page],"html.parser")
    ...
    page += 1
while index < len(strppl):
    umsi_titles[strppl[index]] = strpos[index]
    index += 1
#I HATE ENCODING UGHHHHHH
for name in umsi_titles:
    if "&amp;" in umsi_titles[name]:
        title = umsi_titles[name]
        title_words = title.split()
        title_words = [w.replace('&amp;','&') for w in title_words]
...
def get_five_tweets(query):
    templist = []
    retlist = []
    identifier = "twitter_" + query
    if identifier in CACHE_DICTION:
        print("Using cached data for the search term.\n---------------------")
        results = CACHE_DICTION[identifier]
    else:
        print('Getting cached data for the search term.\n---------------------')
        results = api.search(q=query, rpp = 15)
        CACHE_DICTION[identifier] = results
        f = codecs.open(cachefile_directory,'w')
        f.write(json.dumps(CACHE_DICTION))
    i = 0
    if len(results["statuses"]) < 5:
        print("ATTENTION :: TWEEPY SEARCH RETURNS LESS THAN 5 RESULTS. \nTHIS IS NOT A ERROR IN THE CODE, IT'S JUST THAT THERE AREN'T ENOUGH RESULTS.\nAGAIN, THIS IS NOT THE PROBLEM THAT THE PROGRAMMER MADE, BUT RATHER A SYSTEMATIC SHORTCOMING.\n")
        for statuses in results["statuses"]:
            retlist.append(statuses["text"])
        return retlist
    else:
        i = 0
        for statuses in results["statuses"]:
